...
app.views.AddSauvageOccurenceNonIdentifierView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        this.insertView("#obs-ni-form", new app.views.FormAddOccurenceNIView({
            initialData: this.model
        }));
...
app.views.AddSauvageOccurencePasDansListeView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        this.insertView("#obs-pl-form", new app.views.FormAddOccurencePasDansListeView({
            initialData: this.model
        }));
...
app.views.AddSauvageOccurenceView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        this.insertView("#obs-form", new app.views.FormAddOccurenceView({
            initialData: this.model,
            localisation: this.options.localisation
        }));
...
app.views.AddSauvageRueView = app.utils.BaseView.extend({
    ...
    annulerTerminer: function(evt) {
        app.route.navigate('identification', {
            trigger: true
        });
        return false;
    },
    ...
    beforeRender: function() {
        ...
        this.insertView("#rue-form", new app.views.FormAddSauvageRue({
            initialData: this.model
        }));

        if (typeof(this.collection) !== 'undefined') {
            var currentCollObs = this.collection.findWhere({
                'fk_rue': parseInt(this.model.get('id'),10)
            });
            if (this.collection.length !== 0 && typeof(currentCollObs) !== 'undefined') {
                this.insertView("#rue-obs", new app.views.ObsRueView({
                    collection: this.collection
                }));
            }
        }
...
app.views.ObsRueView = app.utils.BaseView.extend({
    ...
    serialize: function() {
        return {
            collection: this.obsCurrentRue
        };
    },

    beforeRender: function() {
        this.obsCurrentRue = this.collection.where({
            fk_rue: app.globals.currentrue.get('id')
        });
    }
...
app.views.UtilisateurPageView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        $('.icone-page-title').hide();
        this.insertView("#user-form", new app.views.FormUserView({
            initialData: this.model
        }));
    },
...
app.views.RegionPageView = app.utils.BaseView.extend({
    ...
    selectRegion: function(evt) {
        var objCurrentTarget = $(evt.target);
        var idCurrentTarget = objCurrentTarget['context']['id'];
        app.route.navigate('#maregion/' + idCurrentTarget + '', {
            trigger: true
        });
        return false;
    },
...
app.views.IdentificationKeyFilterView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        this.collection.each(function(criteria) {
            this.insertView("#values-list", new app.views.IKCriteriaListItemFilterView({
                model: criteria,
                filtreRegion: this.filtreRegion,
                region: this.region
            }));
        }, this);
...
app.views.IdentificationKeyView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        this.collection.each(function(criteria) {
            this.insertView("#values-list", new app.views.IKCriteriaListItemView({
                model: criteria
            }));
        }, this);
        $('body').addClass('cleliste cle');
...
app.views.TaxonListView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        ...
        var availableLetter = _.uniq(_.map(this.collection.models, function(taxon) {
            return taxon.get("commonName").charAt(0).toUpperCase();
        }));
...
app.views.TaxonDetailView = app.utils.BaseView.extend({
    ...
    beforeRender: function() {
        ...
        var self = this;
        this.collection.each(function(criteria) {
            self.insertView("#values-list", new app.views.IKCriteriaListItemFilterTaxonView({
                model: criteria,
                taxon: self.model
            }));
        }, this);
...
app.views.obsDetailView = app.utils.BaseView.extend({
    ...
    destroyObs: function(event) {
        ...
        this.obsCollection.fetch({
            success: function(dataParcours) {
                var obsToDestroy = self.obsCollection.findWhere({
                    'id': self.collection.models[0].get('id')
                });
                obsToDestroy.destroy({
                    success: function(obs, idObs) {
                        ...
                        app.route.navigate('myObservation', {
                            trigger: true
                        });

                        var obsFkRue = obs.get('fk_rue');
                        var ObsWhereRueTarget = self.obsCollection.where({
                            'fk_rue': parseInt(obsFkRue)
                        })
                        var nbObs = ObsWhereRueTarget.length;

                        var parcoursState = self.parcours.get('state');
                        if (parseInt(nbObs) === 0 && parcoursState !== 0) {
                            self.parcours.destroy({
                                success: function(rue, results) {
                                    app.route.navigate('myObservation', {
                                        trigger: true
                                    });
                                }
...
app.views.ObservationListView = app.utils.BaseView.extend({
    ...
    testConnection: function(event) {
        ...
        this.idRue = parseInt(ctarget.context.id,10);
        ...
        if (connect === 'inconnu' || connect === "none" || connect === false || connect === undefined) {
            ...
            var msg = _.template(
                "<form role='form form-inline'>" +
                "<div class='form-group'>" +
                "       <p>L'envoi des observations requiert une connexion à haut débit (3G,4G, wifi).</p>" +
                "</div>" +
    ...
    destroyRue: function(event) {
        ...
        rueToDestroy.destroy({
            success: function(rue, idRue) {
                if (app.globals.currentrue != undefined && app.globals.currentrue.get('id') == idRue) {
                    app.globals.currentrue = undefined
                };
                ...
                var collectionTarget = self.collection.where({
                    'fk_rue': idRue
                })
    ...
    backRueEnCours: function(event) {
        app.globals.currentrue = this.parcours.findWhere({
            'state': 0
        });
        app.route.navigate('identification', {
            trigger: true
    ...
    supprDrag: function(event) {
        ...
        if (buttonDestroyRue.length === 0) {
            setTimeout(function() {
                $('#' + idParentCurrentTarget[0]['id'] + ' .panel-heading').append('<button type="button" class="confirme-supp btn btn-danger pull-right" id=' + idCurrentTarget + '><span class="glyphicon glyphicon-trash"></span></button>')
            }, 400);
        }
    ...
    findCollectionToTargetEvent: function(event, collection) {
        ...
        var currentCollection = collection.findWhere({
            'id': idCurrentTarget
        });
