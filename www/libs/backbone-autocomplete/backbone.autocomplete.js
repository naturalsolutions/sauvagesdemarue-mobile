...
var AutoCompleteView = Backbone.View.extend({
    ...
    filter: function (keyword) {
        ...
        if (this.model.url) {
            ...
            //CUSTOM
            var self = this;
            app.utils.queryData.findByName(keyword).done(
                function(data) {
                    self.loadResult( data, keyword);
                });
    ...
    select: function (model) {
       // var label = model.label();
        var label = model.nom_sci;
        var numRet = parseInt(model.num_nom);
        this.input.val(label);
        $('input.fk_taxon').val(numRet);
        $('input.name_taxon').val('non renseigné');
